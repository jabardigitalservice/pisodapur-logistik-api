...
class ImportAllocationVaccineRequestController extends Controller
{
     ...
     *
     */
    public function importOld(ImportExcelRequest $request)
    {
        DB::beginTransaction();
        try {
            $import = new MultipleSheetImport();
            ...
            $allocationRequestRule = [
                'letter_number' => 'required|exists:allocation_requests,letter_number',
                'letter_date' => 'required|date',
                'applicant_name' => 'required',
                'applicant_position' => 'required',
                'applicant_agency_id' => 'required|exists:agency,id',
                'applicant_agency_name' => 'required',
            ...
            $validator = Validator::make($allocationRequest, $allocationRequestRule);
            if ($validator->fails()) {
                return response()->format(Response::HTTP_UNPROCESSABLE_ENTITY, $validator->errors());
            }

            $allocationRequest = AllocationRequest::create($allocationRequest);

            // Get distribution allocation requests
            $allocationDistributionRequest = [];
            for ($index = 15; $index < count($allocations); $index++) {
                if ($allocations[$index][1]) {
                    $allocationDistributionRule = [
                    ...
                    $validator = Validator::make($allocationDistribution, $allocationDistributionRule);
                    if ($validator->fails()) {
                        return response()->format(Response::HTTP_UNPROCESSABLE_ENTITY, $validator->errors());
                    }
                    ...
                    for ($column = 0; $column < count($allocations[10])-1; $column++) {
                        if (strpos($allocations[10][$column], 'MAT-') !== false) {
                            $allocationMaterialRequestRule = [
                                'allocation_request_id' => 'required|exists:allocation_requests,id',
                                'allocation_distribution_request_id' => 'required|exists:allocation_distribution_requests,id',
                                'matg_id' => 'required|exists:allocation_materials,matg_id',
                                'material_id' => 'required|exists:allocation_materials,material_id',
                                'material_name' => 'required'
                            ];
                            ...
                            $validator = Validator::make($allocationMaterialRequest, $allocationMaterialRequestRule);
                            if ($validator->fails()) {
                                return response()->format(Response::HTTP_UNPROCESSABLE_ENTITY, $validator->errors());
                            }

                            AllocationMaterialRequest::create($allocationMaterialRequest);
                        }
                    }
                }
            }
